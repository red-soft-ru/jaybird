...
public interface AdvapiLib extends Library {
   ...
   * have been made to CryptoAPI that affect the handling of enveloped messages. For more information,
   * see the Remarks section of CryptMsgOpenToEncode.
   *
   * Important  The CryptEncrypt function is not guaranteed to be thread safe and may return incorrect
   * results if invoked simultaneously by multiple callers.
   *
   * Syntax
   ...
   * This function guarantees that when the same cryptographic service provider (CSP) and algorithms are used, 
   * the keys generated from the same base data are identical. The base data can be a password or any other user data.
   * This function is the same as CryptGenKey, except that the generated session keys are derived from 
   * base data instead of being random. CryptDeriveKey can only be used to generate session keys. 
   * It cannot generate public/private key pairs.
   *
